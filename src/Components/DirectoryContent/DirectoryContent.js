...
class DirectoryContent extends React.Component {
	...
	render () {
		...
		if (this.state.isLoaded && this.state.lastCommitInfo.length) {
			...
			const href = pathName ? `/api/repos/${repositoryName}/tree/master/${pathName}/` : `/api/repos/${repositoryName}/tree/master/`;
			...
			details = this.state.lastCommitInfo.map(item => {
				...
				if (detailsType !== 'folder') iconClass = 'icon icon_script icon_margin_right ';
				...
				return (
					<div className="directory-content-details__item " key={detailsName}>
						<div className="directory-content-details__name ">
							{/* <Link to={href}><span className={iconClass}  onClick={() => this.handleClick(item, href)}/>{detailsName}</Link> */}
